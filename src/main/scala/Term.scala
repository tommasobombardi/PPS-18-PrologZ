...
object Term {
  ...
  object Struct {
    def apply(name: String)(args: Term*): ValidationNel[IllegalArgumentException, Struct] = {
      val nameVal1: ValidationNel[IllegalArgumentException, String] = if(name.isEmpty) new IllegalArgumentException("String representing compound term must be not empty").failureNel else name.successNel
      val nameVal2: ValidationNel[IllegalArgumentException, String] = if(name.exists(c => !c.isLetter)) new IllegalArgumentException("String representing compound term must contain only letter").failureNel else name.successNel
      val nameVal3: ValidationNel[IllegalArgumentException, String] = if(name.nonEmpty && name.charAt(0).isUpper) new IllegalArgumentException("String representing compound term must start with a lowercase letter").failureNel else name.successNel
      val argsVal: ValidationNel[IllegalArgumentException, Seq[Term]] = if(args.isEmpty) new IllegalArgumentException("Argument list in compound term must be not empty").failureNel else args.successNel
