...
object Struct {
  ...
  def apply(name: String): PzValidation[String @@ Functor] = {
    val nameVal1: PzValidation[String] = if(name.nonEmpty) name.successNel
      else InputError("An empty string is not valid to represent a functor").failureNel
    val nameVal2: PzValidation[String] = if(name.toCharArray.forall(_.isLetter)) name.successNel
      else InputError(s"String '$name' is not valid to represent a functor, because it doesn't contain only letters").failureNel
    val nameVal3: PzValidation[String] = if(name.nonEmpty && name.charAt(0).isLower) name.successNel
      else InputError(s"String '$name' is not valid to represent a functor, because it doesn't start with a lowercase letter").failureNel
